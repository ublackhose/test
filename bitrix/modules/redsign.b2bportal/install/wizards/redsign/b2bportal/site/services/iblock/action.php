...
if (COption::GetOptionString("redsign.b2bportal", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA) {
    if ($wizard->GetVar('rewriteIndex', true)) {
        $iblockCode = "redsign_b2bportal_presscenter_promo_" . WIZARD_SITE_ID;
        $iblockType = "presscenter";

        $rsIBlock = CIBlock::GetList(array(), array("XML_ID" => $iblockCode, "TYPE" => $iblockType));
        $iblockID = false;
        if ($arIBlock = $rsIBlock->Fetch()) {
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH . '/action/index.php', ['ACTION_IBLOCK_ID' => $iblockID]);
            CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH . '/_index.php', ['ACTION_IBLOCK_ID' => $iblockID]);
        }
    }
    return;
}
...
$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH . '/xml/' . LANGUAGE_ID . '/action.xml';
...
if ($iblockID == false) {
    ...
    $iblockID = WizardServices::ImportIBlockFromXML(
        $iblockXMLFile,
        "redsign_b2bportal_presscenter_promo",
        $iblockType,
        WIZARD_SITE_ID,
        $permissions
    );
    ...
    $arFields = array(
        ...
        "CODE" => "promo",
        "XML_ID" => $iblockCode,
        'API_CODE' => 'RedsignB2bportalPresscenterPromo' . ucfirst(WIZARD_SITE_ID),
    );
