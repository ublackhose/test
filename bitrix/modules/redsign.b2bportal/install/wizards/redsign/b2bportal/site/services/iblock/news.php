...
if (COption::GetOptionString("redsign.b2bportal", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA) {
    if ($wizard->GetVar('rewriteIndex', true)) {
        $iblockCode = "redsign_b2bportal_presscenter_news_" . WIZARD_SITE_ID;
...
if ($iblockID == false) {
    ...
    $iblockID = WizardServices::ImportIBlockFromXML(
        $iblockXMLFile,
        "redsign_b2bportal_presscenter_news",
        $iblockType,
        WIZARD_SITE_ID,
        $permissions
    );
    ...
    $arFields = array(
        ...
        "CODE" => "news",
        "XML_ID" => $iblockCode,
        'API_CODE' => 'RedsignB2bportalPresscenterNews' . ucfirst(WIZARD_SITE_ID),
    );
