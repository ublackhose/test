...
if(COption::GetOptionString("redsign.b2bportal", "wizard_installed", "N", WIZARD_SITE_ID) != "Y")
{
	$locationGroupID = 0;
	$arLocationArr = Array();
...
if(CModule::IncludeModule('subscribe'))
{
	...
	if(!file_exists($template))
	{
		...
		{
			$size = (int)filesize($fname);

			/** @var string */
			$php_source = fread($fh, $size);
...
if (!$dbResult->Fetch())
{
	...
	if ($groupID > 0)
	{
		...
		if ($editProfileTask)
			CGroup::SetTasks($groupID, [$editProfileTask]);
	}
}
...
COption::SetOptionString("redsign.b2bportal", "wizard_installed", "Y", false, WIZARD_SITE_ID);
