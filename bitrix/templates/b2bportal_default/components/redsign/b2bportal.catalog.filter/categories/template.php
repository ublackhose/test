...
if (
	isset($arResult['SECTIONS']) && isset($arResult['SECTION']) &&
	count($arResult['SECTIONS']) > 0
) {
	...
	$portlet->head(new Portlet\Head(function () {
		...
		$this->title(Loc::getMessage('RS_B2B_CF_CATEGORIES_TITLE') ?: '');
	}));

	$portlet->body(function () use ($arResult, $blockId) {

		$selected = isset($arResult['FIELDS']['SUBSECTIONS']['DATA']['ITEMS']) ? $arResult['FIELDS']['SUBSECTIONS']['DATA']['ITEMS'] : [];
		?>
		<div class="row mb-4">
			<div class="col-9">
				<input id="<?=$blockId?>_search" class="form-control" placeholder="<?=Loc::getMessage('RS_B2B_CF_CATEGORIES_SEARCH')?>">
			</div>
		</div>

		<h6 class="ml-3"><?=$arResult['SECTION']['NAME']?></h6>
		...
		<div id="<?=$blockId?>_tree">
			...
			foreach ($arResult['SECTIONS'] as $arSection)
			{
				...
				?><li data-section-id="<?=$arSection['ID']?>" data-jstree='{"selected":<?=isset($selected[$arSection['ID']]) ? 'true' : 'false'?>}'>
					<a href="#asd"><?=$arSection['NAME']?></a>
				<?php
