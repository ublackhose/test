 ...
 * (такой же код есть в js/component.js для пагинации)
 */
$priceHidden = [
...
foreach ($headers as $key => $headerColumn) {
	if (in_array($headerColumn['field'], $priceHidden)) {
		unset($headers[$key]);	
	}
}
...
foreach ($itemsRows as $key => $item) {
	$priceNew = [];

	foreach ($item['products'][$item['id']]['prices']['ZERO-INF'] as $key2 => $value) {
		if (strpos($key2, 'num') !== false) {
			$priceNew['catalog_price_scale_7_num'] = $value;
		}else{
			$priceNew['catalog_price_scale_7'] = $value;
		}
	}
	
	$itemsRows[$key]['products'][$item['id']]['prices']['ZERO-INF'] = $priceNew;
}
...
?><div findme id="<?=$sBlockId?>_block"><?php $portlet->render(); ?></div>
