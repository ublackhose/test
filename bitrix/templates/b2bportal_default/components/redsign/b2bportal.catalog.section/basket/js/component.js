	  ...
	  return _c("div", {
	    ...
	    return _c("li", {
		  ...
		  _vm.isShowActions ? _c("div", {
			  ...
			  staticClass: "flaticon2-soft-icons"
		  }), _vm._v(" " + "Экспорт каталога" + "\n\t\t")]), _vm._v(" "), _c("div", {
			  ref: "actions",
			  staticClass: "dropdown-menu dropdown-menu-right"
		  }, [_c("ul", {
			  staticClass: "kt-nav kt-nav--fit-ver"
		  }, [_vm.exports.length > 0 ? _c("li", {
			  staticClass: "kt-nav__section"
		  }, [_c("span", {
			  staticClass: "kt-nav__section-text"
		  }, [_vm._v(_vm._s(_vm.localize["export"]))])]) :_c("li", {
			  staticClass: "kt-nav__section"
		  }, [_c("a", {
			  attrs: {
				  href: "/catalog/?export=csv",
				  target: "_blank"

			  },
			  staticClass: "kt-nav__link"
		  }, [_vm._v(_vm._s("Экспорт в CSV"))])]) ], 2)])]) : _vm._e(),
	      ...
	      this.vueTable = new Vue({
	        ...
	        computed: {
	          enablePagination: function enablePagination() {
	            /**
				 * объединение цен в одну
				 * (такой же код есть в template.php для SEO)
				 */
				try {
					let priceHidden = [
						'catalog_price_scale_4',
						'catalog_price_scale_6',
						'catalog_price_scale_5',
					];

					this.data.headers.forEach((item, key) => {
						if (priceHidden.includes(item['field'])) {
							this.data.headers.splice(key, 1); 
						}
					});

					this.data.items.forEach((item, key) => {
						let priceNew = {};
						let priceArray = this.data.items[key].products[item.id].prices['ZERO-INF'];

						for (const key2 in priceArray) {
							if (Object.hasOwnProperty.call(priceArray, key2)) {
								const element = priceArray[key2];
								
								if (key2.includes('num')) {
									priceNew['catalog_price_scale_7_num'] = element;

									$.post('/bitrix/templates/b2bportal_default/ajax/savePriceCatalog.php', {'ID': item.id, 'PRICE': element}, function(data){
										console.log('data')
									});
								}else{
									priceNew['catalog_price_scale_7'] = element;
								}
							}
						}

						this.data.items[key].products[item.id].prices['ZERO-INF'] = priceNew;
					});
